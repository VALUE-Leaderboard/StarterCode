...
from utils.tvr_eval_utils import (
    ...
    get_results_top_n, post_processing_vcmr_nms,
    post_processing_svmr_nms, format_submission_file)
from eval_vcmr import load_model, load_inf_data
...
def main(opts):
    ...
    valid_log, results = validate_full_vcmr(
...
@torch.no_grad()
def validate_full_vcmr(
        model1, val_loader1, split, opts,
        model_opts, task=None, model2=None, 
        val_loader2=None):
    if task is None:
        task = opts.task
    if hasattr(opts, "q2c_alpha"):
        q2c_alpha = opts.q2c_alpha
    else:
        q2c_alpha = model_opts.q2c_alpha
    ...
    for batch in tqdm(val_loader1, desc="Computing q2vScores"):
        ...
        q2video_scores = torch.exp(q2c_alpha * _q2video_scores)

        if val_loader2 is not None:
            _q2video_scores2, _st_probs2, _ed_probs2 =\
                model2.get_pred_from_raw_query(
                    total_frame_embeddings2, total_c_attn_masks2, **batch,
                    cross=True, val_gather_gpus=False)

            _st_probs2 = F.softmax(_st_probs2, dim=-1)
            _ed_probs2 = F.softmax(_ed_probs2, dim=-1)
            _q2video_scores2 = _q2video_scores2.float()
            q2video_scores2 = torch.exp(q2c_alpha * _q2video_scores2)

            q2video_scores = q2video_scores/2.0 + q2video_scores2/2.0
            _st_probs = _st_probs/2.0 + _st_probs2/2.0
            _ed_probs = _ed_probs2/2.0 + _ed_probs/2.0
        ...
        (_sorted_q2c_scores, _sorted_q2c_indices
                ...
                q2video_scores, min(model_opts.max_vcmr_video, len(video_ids)),
                dim=1, largest=True)
    ...
    if has_gt_target:
        ...
        if model_opts.nms_thd != -1 and (
                "VCMR" in opts.full_eval_tasks or
                "SVMR" in opts.full_eval_tasks):
            LOGGER.info(
            ...
            if "SVMR" in gathered_metrics_nms:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--q2c_alpha",
                        default=20, type=float,
                        help="VR predicition weights")
