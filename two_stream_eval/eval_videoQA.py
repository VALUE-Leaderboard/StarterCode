...
from eval_videoQA import (
    load_model, compute_accuracies, load_inf_data)
...
@torch.no_grad()
def validate_videoQA(model1, val_loader1, split, task="tvqa",
    ...
    for i, data in enumerate(dataloader):
        ...
        scores = model1(batch1, "videoQA", compute_loss=False)
        scores = F.softmax(scores, dim=-1)
        if val_loader2 is not None:
            scores2 = model2(batch_2, "videoQA", compute_loss=False)
