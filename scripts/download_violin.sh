...
if [ ! -d $DOWNLOAD/video_db/violin/ ] ; then
    ~/azcopy/azcopy cp $BLOB/video_db/violin.tar $DOWNLOAD/video_db/violin.tar
    tar -xvf $DOWNLOAD/video_db/violin.tar -C $DOWNLOAD/video_db 
    rm $DOWNLOAD/video_db/violin.tar
fi

# text dbs
for SPLIT in 'train' 'val' 'test' 'test_private'; do
    if [ ! -d $DOWNLOAD/txt_db/violin_$SPLIT.db/ ] ; then
