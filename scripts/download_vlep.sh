...
for SPLIT in 'train' 'dev' 'test' ; do
    if [ ! -d $DOWNLOAD/txt_db/vlep_$SPLIT.db/ ] ; then
        wget $BLOB/txt_db/vlep_$SPLIT.db.tar -P $DOWNLOAD/txt_db/
        tar -xvf $DOWNLOAD/txt_db/vlep_$SPLIT.db.tar -C $DOWNLOAD/txt_db
        rm $DOWNLOAD/txt_db/vlep_$SPLIT.db.tar
    fi
done
if [ ! -d $DOWNLOAD/txt_db/vlep_subtitles.db/ ] ; then
    wget $BLOB/txt_db/vlep_subtitles.db.tar -P $DOWNLOAD/txt_db/
    tar -xvf $DOWNLOAD/txt_db/vlep_subtitles.db.tar -C $DOWNLOAD/txt_db
    rm $DOWNLOAD/txt_db/vlep_subtitles.db.tar
fi
