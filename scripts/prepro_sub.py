...
def load_process_sub_meta(sub_meta_path, vid2nframe, frame_length):
    ...
    for vid_name, sub_info in tqdm(
        ...
        # if info["max_unmatched_group_len"] > 40:
        #     import ipdb
        #     ipdb.set_trace()
...
def process_single_vid_sub(sub_listdicts, frame_length, num_of_frames):
    ...
    Returns:
        frame_idx2subtitle_indices:
            dict, {frame_idx: [sub_idx1, sub_idx2, ...]},
    ...
    # if not np.equal(orig_timestamps[:, 0],
    #                 np.sort(orig_timestamps[:, 0])).all():
    #     import ipdb
    #     ipdb.set_trace()
...
def process_tv_subtitles(
        raw_data, video2sub_info, db, tokenizer,
        nframe_group=0):
    vid2len = {}
    ...
    for line in tqdm(raw_data, desc='tokenizing subtitles'):
        ...
        uniq_sub2frames = []
        sub2frames = []
        if "num_of_frames" not in curr_info:
            continue
        ...
        if len(example["sub"]) and n_frames:
            ...
            start_f_inds = list(range(0, n_frames, nframe_group))
            for sub_idx, st_f_idx in enumerate(start_f_inds):
                ed_f_idx = min(st_f_idx+nframe_group, n_frames)
                input_id = []
    ...
    with open_db() as db:
        ...
        with open(opts.annotation) as ann:
            vid2len, vid2max_frame_sub_len = process_tv_subtitles(
                ...
                tokenizer, nframe_group=opts.nframe_group)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nframe_group', default=5, type=int,
                        help='nframe_group if allow_empty_sub')
