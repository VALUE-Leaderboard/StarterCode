...
from .data import (SubOnlyDataset, VideoFeatSubTokDataset, QaQueryTokLmdb,
...
class VideoQaVideoOnlyDataset(VideoQaDataset):
    def __validate_input_db__(self):
        assert isinstance(self.query_db, QaQueryTokLmdb)
        assert isinstance(self.video_db, VideoFeatDataset)


class VideoQaVideoOnlyEvalDataset(VideoQaVideoOnlyDataset):
    def __getitem__(self, i):
        vid, qids = self.getids(i)
        outs = super().__getitem__(i)
        return qids, outs
...
class VideoQaSubOnlyDataset(VideoQaDataset):
    def __validate_input_db__(self):
        assert isinstance(self.query_db, QaQueryTokLmdb)
        assert isinstance(self.video_db, SubOnlyDataset)


class VideoQaSubOnlyEvalDataset(VideoQaSubOnlyDataset):
    def __getitem__(self, i):
        vid, qids = self.getids(i)
        outs = super().__getitem__(i)
        return qids, outs
