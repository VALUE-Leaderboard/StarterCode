...
class VsmDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # quick hack for no matched subs
        if n_samples == 0:
            sub_query_placeholder = torch.tensor([self.vid_sub_db.txt_db.cls_])
            attn_mask_placeholder = torch.ones(
                        len(sub_query_placeholder), dtype=torch.long)
            targets_placeholder = torch.tensor([-1, -1], dtype=torch.long)
            sub_queries_and_targets.append(
                (sub_query_placeholder, attn_mask_placeholder,
                 vid, targets_placeholder))
