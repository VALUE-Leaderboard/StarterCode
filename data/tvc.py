...
from .data import (SubOnlyDataset, VideoFeatSubTokDataset, TxtLmdb,
                   video_collate, _check_ngpu,
                   VideoFeatDataset)


class TvcTokLmdb(object):
...
class TvcTrainDataset(Dataset):
    def __validate_input_db__(self):
        assert isinstance(self.caption_db, TvcTokLmdb)
...
class TvcVideoOnlyTrainDataset(TvcTrainDataset):
    def __validate_input_db__(self):
        assert isinstance(self.caption_db, TvcTokLmdb)
        assert isinstance(self.video_db, VideoFeatDataset)


class TvcVideoOnlyValDataset(TvcValDataset):
    """ for validation """
    def __validate_input_db__(self):
        assert isinstance(self.caption_db, TvcTokLmdb)
        assert isinstance(self.video_db, VideoFeatDataset)
...
class TvcVideoOnlyEvalDataset(TvcEvalDataset):
    """ for generating submission from JSON input
    """
    def __validate_input_db__(self):
        assert isinstance(self.video_db, VideoFeatDataset)
...
class TvcSubOnlyTrainDataset(TvcTrainDataset):
    def __validate_input_db__(self):
        assert isinstance(self.caption_db, TvcTokLmdb)
        assert isinstance(self.video_db, SubOnlyDataset)


class TvcSubOnlyValDataset(TvcValDataset):
    """ for validation """
    def __validate_input_db__(self):
        assert isinstance(self.caption_db, TvcTokLmdb)
        assert isinstance(self.video_db, SubOnlyDataset)
...
class TvcSubOnlyEvalDataset(TvcEvalDataset):
    """ for generating submission from JSON input
    """
    def __validate_input_db__(self):
        assert isinstance(self.video_db, SubOnlyDataset)
