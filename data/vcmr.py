...
from .data import (VideoFeatSubTokDataset, QueryTokLmdb,
                   ...
                   VideoFeatDataset, SubOnlyDataset)


class VcmrDataset(Dataset):
    def __validate_input_db__(self):
        assert isinstance(self.query_db, QueryTokLmdb)
        assert isinstance(self.video_db, VideoFeatSubTokDataset)

    def __get_vid2idx__(self):
        if len(self.video_db.vid2dur):
...
class VcmrVideoOnlyDataset(VcmrDataset):
    ...
    def __get_vid2idx__(self):
        self.global_vid2idx = self.video_db.vid2idx
        self.vid2dur = self.video_db.vid2dur
        self.vid2idx = {
            vid_name: self.global_vid2idx[vid_name]
            for vid_name in self.vids}
...
class VcmrVideoOnlyFullEvalDataset(VcmrFullEvalDataset):
    def __validate_input_db__(self):
        assert isinstance(self.query_db, QueryTokLmdb)
        assert isinstance(self.video_db, VideoFeatDataset)
...
class VcmrSubOnlyDataset(VcmrDataset):
    def __validate_input_db__(self):
        assert isinstance(self.query_db, QueryTokLmdb)
        assert isinstance(self.video_db, SubOnlyDataset)


class VcmrSubOnlyEvalDataset(VcmrSubOnlyDataset):
    def __getitem__(self, i):
        vid, qids = self.getids(i)
        outs = super().__getitem__(i)
        return qids, outs
...
class VcmrSubOnlyFullEvalDataset(VcmrFullEvalDataset):
    def __validate_input_db__(self):
        assert isinstance(self.query_db, QueryTokLmdb)
        assert isinstance(self.video_db, SubOnlyDataset)
