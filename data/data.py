...
class VideoFeatLmdb(object):
    def __init__(self, img_dir, feat_version="resnet_slowfast",
        ...
        db_name = f'{feat_version}_{frame_interval:g}'
        try:
            self.name2nframe = load_json(
                f'{img_dir}/id2nframe_{frame_interval:g}.json')
        except Exception:
            self.name2nframe = load_json(
                f'{img_dir}/id2nframe.json')
...
class TxtTokLmdb(object):
    def __init__(self, db_dir, max_txt_len=60):
        ...
        meta = load_json(f'{db_dir}/meta.json')
        self.cls_ = meta['CLS']
        self.eos = meta['EOS']
        self.pad = meta['PAD']
        self.bos = meta['BOS']
        ...
        if os.path.exists(id2len_path):
            if max_txt_len == -1:
                self.id2len = load_json(f'{db_dir}/id2len.json')
            else:
                self.id2len = {
                    id_: len_
                    for id_, len_ in load_json(
                        f'{db_dir}/id2len.json').items()
...
class SubTokLmdb(TxtTokLmdb):
    def __init__(self, db_dir, max_clip_len=-1):
        ...
        if os.path.exists(video_data_file):
            video_data = load_json(video_data_file)
...
class VideoFeatSubTokDataset(Dataset):
    def __validate_input_db__(self):
        assert isinstance(self.txt_db, SubTokLmdb)
        assert isinstance(self.img_db, VideoFeatLmdb)
    ...
    def __getitem__(self, vid_):
        ...
        frame_level_sub_attn_masks = []  # [(www)]
        ...
        for sub_idx, matched_frames in sub2frames:
            ...
            frame_level_sub_attn_masks.append(
                torch.tensor([1] * len(input_ids)))
        ...
        out = (frame_level_input_ids,   # num_subs list[tensor(sep,w0,w1,...)]
               ...
               frame_level_sub_attn_masks,  # num_subs list[L_sub]
...
def video_collate(inputs):
    ...
    for i in range(len(num_subs)):
        ...
        all_f_sub_attn_masks += frame_level_sub_attn_masks[i]
    ...
    all_f_sub_attn_masks = pad_sequence(
        all_f_sub_attn_masks, batch_first=True, padding_value=0)
    ...
    batch = {'f_sub_input_ids': all_f_sub_input_ids,  # (total_sub, max_sl)
             ...
             'f_sub_attn_masks': all_f_sub_attn_masks,        # (total_sub, max_sl)
...
class VideoFeatDataset(Dataset):
    ...
    def __getitem__(self, vid_):
        ...
        frame_level_input_ids, frame_level_v_feats = (
            ...
            [v_feat])
        frame_level_sub_attn_masks = [torch.tensor([1])]
        frame_level_attn_masks = [
...
class SubOnlyDataset(VideoFeatSubTokDataset):
    def __getitem__(self, vid_):
        '''
        matched_sub_frames:
        [[txt, txt_position_ids, v_feat(frames), v_position_ids, attn_masks],
        ...
        for sub_idx, matched_frames in sub2frames:
            # text input
            input_ids = []
            input_ids = [self.txt_db.cls_] + input_ids
            for tmp_sub_idx in range(sub_idx-self.sub_ctx_len,
                                     sub_idx+1):
                if tmp_sub_idx >= 0 and tmp_sub_idx < num_subs:
                    input_ids.extend(
            ...
            if len(matched_frames):
                matched_v_feats = torch.index_select(
                    v_feat, 0, torch.tensor(matched_frames))
                attn_masks = [0]*len(matched_frames) + [1] * (len(input_ids))
            else:
                matched_v_feats = torch.zeros(1, v_feat.shape[1])
                attn_masks = [0] + [1] * len(input_ids)
