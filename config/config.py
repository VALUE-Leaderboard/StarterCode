...
def parse_with_config(parser):
    ...
    if args.config is not None:
        config_args = load_json(args.config)
...
class SharedConfigs(object):
    ...
    def __init__(
        ...
        parser.add_argument("--vfeat_version",
                            ...
                            choices=[
                                "resnet_slowfast", "resnet_mil-nce", "resnet",
                                "clip-vit_slowfast", "slowfast",
                                "mil-nce", "clip-vit", "clip-vit_mil-nce"],
        ...
        parser.add_argument(
            ...
            help="Whether to use 16-bit float precision instead "
                 "of 32-bit, disable fp16, use fp32 instead")

        # can use config files
        parser.add_argument('--config', help='JSON config files')
        
        parser.add_argument("--load_partial_pretrained",
                            action='store_true',
                            help="load only for frame embedding model")
        parser.add_argument("--skip_layer_loading",
                            action='store_true',
    ...
    def get_vsm_args(self):
        ...
        self.parser.add_argument(
                "--hard_negative_start_step", type=int, default=[10000],
                nargs='+',
    ...
    def get_pretrain_args(self):
        ...
        assert len(args.hard_negative_start_step) == len(
    ...
    def get_videoCap_args(self):
        
        self.parser.add_argument(
            '--max_cap_per_vid', type=int, default=-1,
            help='max number of captions per video')
        self.parser.add_argument(
            '--lsr', default=0.1, type=float,
            help='label smoothing regularization')
        self.parser.add_argument(
            '--max_gen_step', type=int, default=30,
            help='max number of generation step')

        args = self.parse_args()
        return args

    def get_multi_task_args(self):
        self.parser.add_argument("--vcmr_eval_video_batch_size",
                                 default=40, type=int,
                                 help="Total video batch size for validation. "
                                      "(batch by number of videos)")
        ...
        # video QA
        self.parser.add_argument(
            "--qa_lw_st_ed", type=float, default=1,
            help="weight for st ed prediction loss")
