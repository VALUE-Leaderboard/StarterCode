...
run evaluation of VCMR or infenrece for submission
1. TVR
2. How2R
...
def load_model(model_dir, checkpoint, opts, device):
    ...
    model = HeroForVcmr.from_pretrained(
        ...
        lw_neg_q=model_opts.lw_neg_q, lw_st_ed=model_opts.lw_st_ed,
...
@torch.no_grad()
def validate_full_vcmr(model, val_loader, split, opts, model_opts, task=None):
    if task is None:
        task = opts.task
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", choices=["tvr", "how2r"],
                        default="tvr", type=str,
                        help="The evaluation vcmr task")
    parser.add_argument("--vid2dur_idx",
                        default="/txt/tv_subtitles.db/vid2dur_idx.jsonl",
                        type=str, help="vid2dur_idx used for vcmr evaluation")
