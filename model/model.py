...
from .modeling_utils import (
    load_pretrained_weight, load_partial_checkpoint)
from .embed import (
    SubEmbeddings, ImageEmbeddings)
...
class VideoModelConfig(object):
    def __init__(self, config_json_file):
        ...
        assert self.model in [
            "hero", "video_sub_feature_fusion",
            "video_sub_sequence_model"]
        if "video_sub_fusion_method" in config:
            self.video_sub_fusion_method = config["video_sub_fusion_method"]
            assert self.video_sub_fusion_method in ["concat", "add"]
            assert self.model == "video_sub_feature_fusion"
        else:
            self.video_sub_fusion_method = None
...
class VideoPreTrainedModel(RobertaPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        RobertaPreTrainedModel.__init__(self, config.f_config)
...
class FrameFeatureRegression(nn.Module):
    def __init__(self, hidden_size, feat_dim):
        super(FrameFeatureRegression, self).__init__()
...
class HierarchicalVlModel(VideoPreTrainedModel):
    def __init__(self, config, vfeat_dim, max_frm_seq_len,
        ...
        VideoPreTrainedModel.__init__(self, config)
    ...
    def collect_frame_outputs(self, out_shape, frame_sequence_output,
        ...
        for vid, num_sub in enumerate(num_subs):
            ...
            for sid, matched_frame_idx in current_sub_n_f:
                ...
                if sub_only:
                    # get the cls token repr
                    fused_frame_repr = frame_sub[sid, n_frame]
                else:
                    fused_frame_repr = frame_sub[sid, :n_frame]
...
class HeroModel(VideoPreTrainedModel):
    def __init__(self, config, vfeat_dim, max_frm_seq_len):
        VideoPreTrainedModel.__init__(self, config)
        self.config = config
        if self.config.model == "video_sub_feature_fusion":
            self.v_encoder = Video_Sub_Feature_Fusion(
                config, vfeat_dim, max_frm_seq_len)
        elif self.config.model == "video_sub_sequence_model":
            self.v_encoder = Video_Sub_Sequence_Model(
                config, vfeat_dim, max_frm_seq_len)
        else:
    ...
    def load_partial_pretrained(self, checkpoint, vfeat_dim, max_frm_seq_len,
        ...
        ckpt_vocab_size = partial_checkpoint[
            "roberta.embeddings.word_embeddings.weight"].shape[0]
        import copy
        f_config = copy.deepcopy(self.config.f_config)
        if (f_config.vocab_size > ckpt_vocab_size and
                f_config.vocab_size % 8 == 0):
            f_config.vocab_size = ckpt_vocab_size
        if (f_config.type_vocab_size != 1):
            f_config.type_vocab_size = 1
        self.v_encoder.f_encoder = CrossModalTrm.from_pretrained(
            f_config, state_dict=partial_checkpoint,
            vfeat_dim=vfeat_dim, max_img_seq_len=max_frm_seq_len)
        self.v_encoder.f_encoder.pad_vocab()
        self.v_encoder.f_encoder.init_type_embedding()


class Video_Sub_Feature_Fusion(HierarchicalVlModel):
    def __init__(self, config, vfeat_dim, max_frm_seq_len):
        VideoPreTrainedModel.__init__(self, config)
        # f_encoder is only used to encode subtitle text in H_TRM
        self.f_encoder = CrossModalTrm(
            config.f_config, vfeat_dim, max_frm_seq_len)
        self.frame_transform = LinearLayer(
            vfeat_dim, config.f_config.hidden_size,
            layer_norm=True, dropout=config.f_config.hidden_dropout_prob,
            relu=True)
        self.sub_transform = LinearLayer(
            config.f_config.hidden_size, config.f_config.hidden_size,
            layer_norm=True, dropout=config.f_config.hidden_dropout_prob,
            relu=True)
        self.c_encoder = TemporalTrm(
            config.c_config)
        self.video_sub_fusion_method = config.video_sub_fusion_method
        assert self.video_sub_fusion_method in ["add", "concat"]
        if self.video_sub_fusion_method == "concat":
            self.video_sub_feature_transform = LinearLayer(
                config.f_config.hidden_size*2, config.f_config.hidden_size,
                layer_norm=True, dropout=config.f_config.hidden_dropout_prob,
                relu=True)
    ...
    def forward(self, batch, task='repr', compute_loss=False):
        batch = defaultdict(lambda: None, batch)
        if task == 'repr':
            # get frame representations
            return self.forward_repr(batch)
            ...
            raise ValueError(
                ...
                f'note that pre-training on Video_Sub_Feature_Fusion'
                ' is not implemented')

    def collect_sub_outputs(self, out_shape, sub_sequence_output,
                            num_subs, sub_idx2frame_idx):
        """
        Inputs:
        :out_shape              (bz, #frames, hidden_size)
        :sub_sequence_output  tensor (total_subs, max_sl, hidden_size)
        :num_subs               [num_sub]
        :sub_idx2frame_idx      [ [(sub_ix, [frame_ix])] ]
        Return:
        :matched_v_feats        tensor (bz, #frames, hidden_size)
        """
        sub_feats = torch.zeros(*out_shape,
                                dtype=sub_sequence_output.dtype,
                                device=sub_sequence_output.device)
        start_idx, end_idx = 0, 0
        for vid, num_sub in enumerate(num_subs):
            current_sub_n_f = sub_idx2frame_idx[vid]
            end_idx += num_sub
            curr_video_subs = sub_sequence_output[start_idx: end_idx]
            for sid, matched_frame_idx in current_sub_n_f:
                n_frame = len(matched_frame_idx)
                if n_frame == 0:
                    continue
                matched_frame_idx = torch.tensor(
                    matched_frame_idx, dtype=torch.long,
                    device=curr_video_subs.device)
                curr_sub_repr, _ = torch.max(curr_video_subs[sid], dim=0)
                sub_feats[vid, matched_frame_idx] =\
                    sub_feats[vid, matched_frame_idx] +\
                    curr_sub_repr
    ...
    def forward_repr(self, batch, encode_clip=True):
        ...
        sub_batch = {}
        sub_batch['input_ids'] = batch["f_sub_input_ids"]
        sub_batch['attn_masks'] = batch["f_sub_attn_masks"]
        sub_batch = defaultdict(lambda: None, sub_batch)
        batched_sub_output = self.f_encoder(sub_batch, 'txt')
        # (total_sub, sub_len, 768)
        sub_sequence_output = batched_sub_output[0]

        c_v_feats = batch['c_v_feats']
        assert torch.sum(c_v_feats) != 0
        c_attn_masks = batch['c_attn_masks']
        num_subs = batch['num_subs']
        sub_idx2frame_idx = batch['sub_idx2frame_idx']

        # (bz, #frames, 768)
        shape = list(c_v_feats.size()[:2]) + [sub_sequence_output.size(-1)]
        matched_sub_feats = self.collect_sub_outputs(
            shape, sub_sequence_output, num_subs, sub_idx2frame_idx)

        # residual connection (transformed_v_feat = raw_v_feat + fused_v_feat)
        transformed_v_feats = self.frame_transform(c_v_feats)
        matched_sub_feats = self.sub_transform(matched_sub_feats)
        if self.video_sub_fusion_method == "concat":
            v_sub_concat_feats = torch.cat(
                (transformed_v_feats, matched_sub_feats), dim=-1)
            transformed_v_feats = self.video_sub_feature_transform(
                v_sub_concat_feats)
        else:
            transformed_v_feats = transformed_v_feats + matched_sub_feats

        if encode_clip:
            # compute pos_ids in embedding layer
            clip_outputs = self.c_encoder(
                clip_level_pos_ids=None,
                clip_level_frame_feat=transformed_v_feats,
                attention_mask=c_attn_masks)
            return clip_outputs
        return transformed_v_feats
...
class Video_Sub_Sequence_Model(Video_Sub_Feature_Fusion):
    def __init__(self, config, vfeat_dim, max_frm_seq_len):
        VideoPreTrainedModel.__init__(self, config)
        # f_encoder is used to encode [vfeat, sub_txt_embedding] in F_TRM
        self.f_encoder = CrossModalTrm(
            config.f_config, vfeat_dim, max_frm_seq_len)
        self.frame_transform = LinearLayer(
            vfeat_dim, config.f_config.hidden_size,
            layer_norm=True, dropout=config.f_config.hidden_dropout_prob,
            relu=True)
        self.sub_embeddings = SubEmbeddings(
            config.f_config)

        self.c_encoder = TemporalTrm(
            config.c_config)
        self.frame_embeddings = ImageEmbeddings(
            config.c_config, vfeat_dim, max_frm_seq_len)

    def transform_frame_sub_features(self, frame_features, sub_features):
        transformed_sub_features = self.sub_embeddings(
            inputs_embeds=sub_features)
        frame_type_embeddings = self.sub_embeddings.token_type_embeddings(
            torch.ones(1, 1, dtype=torch.long,
                       device=frame_features.device))
        transformed_frame_features = self.frame_embeddings(
            frame_features, frame_type_embeddings)
        return transformed_frame_features, transformed_sub_features

    def collect_sub_sentence_embeddings(
            self, sub_sequence_output, num_subs):
        """
        Inputs:
        :out_shape              (bz, #frames, hidden_size)
        :sub_sequence_output  tensor (total_subs, max_sl, hidden_size)
        ...
        bsz = len(num_subs)
        hid_size = sub_sequence_output.shape[-1]
        max_num_sub = max(num_subs)

        sub_embeddings = torch.zeros(
            bsz, max_num_sub, hid_size, dtype=dtype, device=device)
        sub_attn_masks = torch.zeros(
            bsz, max_num_sub, dtype=torch.long, device=device)

        start_idx, end_idx = 0, 0
        for vid, num_sub in enumerate(num_subs):
            end_idx += num_sub
            curr_sub_sentences = sub_sequence_output[
                start_idx: end_idx]  # (num_subs, num_sub_toks, hid_zie)
            if not len(curr_sub_sentences) or num_sub == 0:
                continue
            curr_sub_embeddings, _ = torch.max(curr_sub_sentences, dim=1)
            # auto-broadcasted
            sub_embeddings[vid, :num_sub, :] =\
                sub_embeddings[vid, :num_sub, :] + curr_sub_embeddings
            sub_attn_masks[vid, :num_sub] = sub_attn_masks[vid, :num_sub] + 1

            start_idx = end_idx
        return sub_embeddings, sub_attn_masks
    ...
    def forward_repr(self, batch, encode_clip=True):
        ...
        c_attn_masks = batch['c_attn_masks']
        num_subs = batch['num_subs']
        num_frames = c_v_feats.shape[1]

        # (bz, #subs, 768)
        (sub_sentence_feats, sub_attn_masks
         ) = self.collect_sub_sentence_embeddings(
            sub_sequence_output, num_subs)

        (transformed_v_feats, transformed_sub_feats
         ) = self.transform_frame_sub_features(c_v_feats, sub_sentence_feats)

        v_sub_sequnece_concat = torch.cat(
            (transformed_v_feats, transformed_sub_feats), dim=1)
        v_sub_attn_masks = torch.cat((c_attn_masks, sub_attn_masks), dim=1)

        if encode_clip:
            v_sub_sequence_output = self.c_encoder.forward_encoder(
                v_sub_sequnece_concat, v_sub_attn_masks)
            # (bz, #frames, 768)
            clip_output = v_sub_sequence_output[:, :num_frames, :]
            return clip_output
        return v_sub_sequnece_concat, v_sub_attn_masks
