...
class HeroForVr(HeroForVcmr):
    ...
    def forward(self, batch, task='msrvtt_video_sub', compute_loss=True):
        if task in [
                'msrvtt_video_sub', 'msrvtt_video_only',
                'yc2r_video_sub', 'yc2r_video_only',
                'vatex_video_only', 'vatex_video_sub']:
            if compute_loss:
                _, loss_neg_ctx, loss_neg_q = super(HeroForVr, self).forward(
                    batch, task='tvr', compute_loss=True)
                return loss_neg_ctx, loss_neg_q
            else:
                q2video_scores, _, _ = super(HeroForVr, self).forward(
