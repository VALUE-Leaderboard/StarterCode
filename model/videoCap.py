...
PyTorch Modules for Video Captioning, shared by:
1. TVC
2. VATEX_EN_C
3. YC2C
...
class HeroForVideoCap(HeroModel):
    def __init__(self, config, vfeat_dim, max_frm_seq_len, lsr=0.1):
        HeroModel.__init__(self, config, vfeat_dim, max_frm_seq_len)
        self.config = config
...
class VideoCapGenerator(object):
    ...
    def greedy_decode(self, batch):
        ...
        encoder_outputs, enc_mask = self.model.encode(batch)  # (N, Lv, D)
        if self.fp16:
